...
Purpose:               This constructor function allocates and initializes a new
		       Stack object. It allocates memory to hold stacksize 
...
Description:	       Creating a memory that takes in a stacksize that the 
		       ...
		       Every time the user creates a new stack the stack_counter
		       will increase.
...
Stack * new_Stack (unsigned long stacksize) {
	...
	if (debug == TRUE) {
		/*prints out the debug message with the number of stack the user
		 * has created */
...
long pop (Stack * this_Stack, long * item) {
	...
	/* the item will now be the number that was stored in the stack array */
	* item = this_Stack[sp];  
	...
	if (debug == TRUE) {
		...
		fprintf(stderr, POP, (long)stack_counter,
		* item);
	}
...
Input:                 Stack * this_Stack: A pointer to the current stack that
		       ...
		       the program.
Result:                Returns false if a stack has not been created by the user
		       or if the stack is full, which causes the user to not 
		       be able to push any numbers into the stack array. 
...
long push (Stack * this_Stack, long item) {
	...
	if (debug == TRUE) {
		...
		fprintf(stderr, PUSH, (long)stack_counter,
		item);
	}
